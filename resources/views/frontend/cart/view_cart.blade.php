...
<section class="section pt-4 pb-4 osahan-account-page">
    ...
    <tr>
        <td>
            <img src="{{ asset($item['image']) }}" width="60" height="60" class="rounded" alt="produk">
        </td>
        <td>{{ $item['name'] }}</td>
        <td>{{ $item['client_id'] ? 'Vendor' : 'Owner' }}</td>
        <td>{{ $item['id'] }}</td> <!-- bisa pakai id produk sebagai code -->
        <td>{{ $item['quantity'] }}</td>
        <td>
            Rp {{ number_format($item['price'], 0, ',', '.') }} <br>
            <small>Total = Rp {{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }}</small>
        </td>
    </tr>
@endforeach
                            ...
                            <div class="text-end">
                                <h5 class="fw-bold mt-3">Subtotal: Rp {{ number_format($subtotal, 0, ',', '.') }}</h5>
<h5 class="fw-bold mt-3">Total Setelah Kupon: Rp {{ number_format($finalAmount, 0, ',', '.') }}</h5>
<h5 class="fw-bold mt-3">Total Harga: Rp {{ number_format($finalAmount, 0, ',', '.') }}</h5>
                            </div>
                        </div>
                    </div>
